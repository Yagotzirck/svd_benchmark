...
%SVD_CUSTOM Returns the singular values and the singular vectors of the
%matrix A (full-rank SVD; same functionality as
...
%
%       eps_sigma - Any squared singular value below this threshold
%                   is considered zero and discarded, along with the
%                   singular vectors associated to them.
%
%   Given:
%       m: A's rows;
%       n: A's columns;
...
%CALC_SINGULAR calculates the singular values s and singular vectors X,
%where X are:
%   - the left singular vectors U, if AA = A * A';
%   - the right singular vectors V, if AA = A' * A.
[AA,H] = tridiag(AA);
